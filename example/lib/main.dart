...
//                     const GFade().tween(const FlutterLogo(size: 50)),
//
...
class _BenchmarkState extends State<Benchmark> {
  double _tweenSliderValue = .1;
  bool _enableRendering = true;

  int get _tweenCount => 10 + (_tweenSliderValue * 1000).round();
  @override
  Widget build(BuildContext context) {
    final rnd = Random(0);
    return Center(
      child: SizedBox(
        ...
        child: Stack(children: [
          ...List.generate(_tweenCount, (index) {
            return Positioned(
                ...
                child: Opacity(
                    ...
                    child: const FlutterLogo().gTweener.fade().withInit((controller) {
          ...
          Column(
            children: [
              CheckboxListTile(
                  title: Text('Show Tweens'), value: _enableRendering, onChanged: _handleShowTweensToggled),
              Row(
                children: [
  ...
  void _handleShowTweensToggled(_) {
    setState(() => _enableRendering = !_enableRendering);
  }
}

...
class GRotate extends GTween<double> {
  const GRotate({double from = 0, double to = 0, Curve? curve}) : super(from: from, to: to, curve: curve);
  static const degreesToRad = 0.0174533;
  @override
  Widget build(Widget child, Animation<double> anim) =>
      Transform.rotate(angle: tweenAndCurveAnim(anim).value * degreesToRad, child: child);
}
